// Adapted from uzen-zone/kokoro-js, revision bbc3b5da19965257db0c8e6c9fb9381175b165a7.
// Local changes: source-span callbacks, normalization exports and rhotic-schwa mapping.
 ...
 * @returns {string}
 */
function phonemize_zh_text(text, onWord) {
  const parts = [];
  let index = 0;
  ...
  while (index < text.length) {
    ...
    if (matchedPhrase) {
      ...
      onWord?.(index, index + matchedPhrase.length, CHINESE_PHRASE_OVERRIDES.get(matchedPhrase));
    ...
    const phonemes = merged.map((seg) => {
      const start = text.indexOf(seg, wordOffset);
      if (start < wordOffset) throw new Error('中文音素无法映射回文本：' + seg);
      wordOffset = start + seg.length;
      const ps = phonemize_zh_word(seg);
      onWord?.(start, wordOffset, ps);
      return ps;
    }).join("/");
    if (phonemes) {
      parts.push(phonemes);
...
export async function phonemize(text, language = "a", norm = true) {
  ...
  let processed = ps
    ...
    .replace(/ʲ/g, "j")
    // Misaki's EspeakFallback uses the same mapping for Kokoro's vocabulary:
...
// Local addition: expose the existing normalization rules and contextual Chinese
// frontend so the extension can retain source spans without re-phonemizing words.
export { normalize_text, normalize_chinese_numbers, normalize_chinese_punctuation, phonemize_zh_text };
